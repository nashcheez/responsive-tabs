...
function tabConfiguration() {
    ...
    this.tabClickRequestFunction = ""; // define click callback function for all tabs
...
$(window).resize(function() {
    ...
    if (bCurrentMobile !== common.bIsMobile) {
        ...
        if (bCurrentMobile) {
            $(".tabsMainContainer ul")
                .removeClass("toggleTabsExpanded")
                .addClass("toggleTabsCollapsed");
        }
    }
});
...
function isScreenMobile() {
    var bCurrentMobile = common.bIsMobile,
        iWidth;
...
function getWindowWidth() {
    ...
    if (
        sUserAgent.indexOf("windows") >= 0 &&
        sUserAgent.indexOf("safari") >= 0 &&
        sUserAgent.indexOf("chrome") === -1
    ) {
        iWidth = $(window).width();
    } else {
